...
path_CVE_fixes_ffmpeg = r'D:\pycharm\projects\Capstone\datasetsNew\FFmpeg\CVE_PostfixFiles\**\**\*.c'
path_CVE_flaws_ffmpeg = r'D:\pycharm\projects\Capstone\datasetsNew\FFmpeg\CVE_PrefixFiles\**\**\*.c'

doc1 = pd.concat((pd.read_csv(f,sep='\t',engine='python') for f in glob.iglob(path_CVE_fixes_ffmpeg, recursive=True)), ignore_index=True)
doc2 = pd.concat((pd.read_csv(f,sep='\t',engine='python') for f in glob.iglob(path_CVE_flaws_ffmpeg, recursive=True)), ignore_index=True)
